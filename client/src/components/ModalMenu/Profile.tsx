...
import { changeAvatar, editUser } from "../../redux/action-creators/auth.action-creator";
...
const Profile:React.FC = () => {
  ...
  const handleSelectImage = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.currentTarget?.files && e.currentTarget.files[0]) {
      ...
      reader.onload = () => {
         if (typeof reader.result === 'string') dispatch(changeAvatar(reader.result));
      }
      reader.onerror = () => {
        console.log(reader.error);
        dispatch(modalActions.error('Error while trying to load image'));
      }
    }
   }, [dispatch]);
